...
export default function Homepage() {
  ...
  const renderItems = () => {
    return itemData.map((item, index) => (
      <div key={index} style={{ '--offset': `${index - 1}` }} className={`${index === itemData.length - 1 && 'col-span-full mb-2'} ${areCardsStacked ? "w-[70%] md:w-[50%] hover:example-item-popped" : "w-full"} font-inter bg-card rounded-lg p-6 md:p-8 lg:p-10 flex flex-col justify-center items-center example-item cursor-pointer relative`} onClick={() => navigateTo(item.url)}>
        <h1 className={`${index === currentAnimation && !areCardsStacked ? '-translate-y-[50%] lg:-translate-y-[100%]' : ''} text-xl md:text-2xl example-item-title ${index === 0 && 'animate-typing inline-block justify-between overflow-hidden whitespace-nowrap mx-auto'}`} style={{
